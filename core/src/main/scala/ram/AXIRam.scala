...
case class AXIRamConfig(dataWidth: Int = 32, addrWidth: Int = 16,
                        idWidth: Int = 8, pipelineOutput: Int = 0) {
...
class axi_ram(config: AXIRamConfig) extends BlackBox {
  ...
  val io = new Bundle {
    val axi = slave(new Axi4(Axi4Config(
      addressWidth = config.addrWidth, dataWidth = config.dataWidth, idWidth = config.idWidth,
