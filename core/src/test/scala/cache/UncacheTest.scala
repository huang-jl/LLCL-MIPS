...
class UncacheTest extends Component {
  ...
  val uncache = new Uncache()
  val ram = new axi_ram(AXIRamConfig(32, 16, 4))

  io.cpu >> cpu
  cpu >> uncache.io.cpu
  uncache.io.axi >> ram.io.axi
}
  ...
  def main(args: Array[String]): Unit = {
    ...
    compile.doSim("Uncached_read_after_write", 2021) { dut =>
      ...
      for (i <- 0 to 200) {
        ...
        dut.clockDomain.waitFallingEdge()
        while (dut.io.cpu.stall.toBoolean) (dut.clockDomain.waitSampling())
        assert(dut.io.cpu.rdata.toLong == randData)
        dut.io.cpu.read #= false
        dut.io.cpu.write #= false
      }
    }

    compile.doSim("uncache_small_range_random_read_write", 2021) { dut =>
      dut.clockDomain.forkStimulus(10)
      dut.clockDomain.assertReset()
      sleep(10)
      dut.clockDomain.deassertReset()
    ...
    compile.doSim("uncache_small_range_byteEnable", 2021) { dut =>
      dut.clockDomain.forkStimulus(10)
      dut.clockDomain.assertReset()
      sleep(10)
      dut.clockDomain.deassertReset()
      ...
      for (i <- 0 to 30000) {
        val op = rand.nextInt(2) //0: read, 1: write
        var randomRamAddr = rand.nextInt(0x600) & 0xfffc
        var waitCycle = rand.nextInt(3)
        if (i == 0 && waitCycle == 0) waitCycle = 1
        if (op == 0) {
          val cpuAddr = (0xbfc00000L + randomRamAddr) & ((0x1L << 32) - 1)
