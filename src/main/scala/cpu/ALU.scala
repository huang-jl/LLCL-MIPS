...
class SimpleALU extends Component {
  val op = in(ALU_OP())
  ...
  switch(op) {
    ...
    is(xor) { c := a ^ b }
    is(nor) { c := ~(a | b) }
    is(sll) { c := b |<< a(4 downto 0) }
    is(lu) { c := b |<< 16 }
    is(srl) { c := b |>> a(4 downto 0) }
    is(sra) { c := U(S(b) >> a(4 downto 0)) }
    is(slt) { c := U(S(a) < S(b), 32 bits) }
    is(sltu) { c := U(a < b, 32 bits) }
    default { c.assignDontCare() }
  }
}
