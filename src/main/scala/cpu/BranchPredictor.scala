...
class BranchPredictor(numStages: Int) extends Component {
  val pc = Key(Address())
  ...
  val io = new Bundle {
    val pc = in(Address())
    val nextPC = in(Address())

    val isDJump1 = in(Bool())
    val jumpPC1 = in(UInt(32 bits))

    val isDJump2 = in(Bool())
    val jumpPC2 = in(UInt(32 bits))

    val jump = in(Bool())

    val assignJump = out Bool()
  ...
  val write1 = new ComponentStage {
    ...
    bpu.io.w.bhtV := stored(bhtV)(BHT.DATA_WIDTH - 1 downto 1) ## io.jump
...
object BranchPredictor {
  def main(args: Array[String]): Unit = {
    SpinalVerilog(new BranchPredictor(5))
  }
}